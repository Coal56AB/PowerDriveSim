...
Проект развивается поэтапно. Работают редактор атомарных и иерархических схем, численное ядро, библиотечные преобразователи, идеальные трансформаторы и двигатель постоянного тока. Другие электрические машины и ускоренные расчёты находятся в плане разработки.
...
- однофазный и трёхфазный Y–Y трансформаторы, двигатель постоянного тока с выходом скорости;
- редактируемые блоки кода, ПИ/ПИД-регуляторы и ШИМ для полумоста, 2L и 3L инверторов;
...
- [Трёхфазный трансформатор Y–Y](examples/transformer-yy.pds) с отдельными нейтралями.
- [Разгон двигателя](examples/dc-motor-startup.pds) и [замкнутое управление скоростью](examples/dc-motor-pwm.pds) через ПИ-регулятор и ШИМ-полумост.
...
Приложение находится в `build/Release/powerdrive-desktop.exe`. Windows-сборка копирует нужные библиотеки Qt рядом с ним; [команды сборки и запуска](docs/desktop.md#сборка-и-запуск) приведены отдельно. Локальную сборку можно выполнить двойным щелчком по `build-windows.cmd`; путь к нестандартной установке Qt задаётся переменной `PDS_QT_ROOT`.

Установщик ещё не выпущен. Запуск без графического интерфейса также поддерживается.

## Дальнейшее развитие

Дальнейшие этапы включают механические нагрузки и другие машины, ускорение расчётов и импорт внешних моделей.
