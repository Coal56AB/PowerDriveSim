...
std::string Document::add_component(Kind kind,double x,double y) {
    ...
    apply("Add component",[&](Project& p) {
        double value=kind==Kind::resistor?1000:(kind==Kind::dc_motor?2:(kind==Kind::capacitor?1e-6:(kind==Kind::inductor?.01:(kind==Kind::voltage||kind==Kind::ideal_transformer?1:(kind==Kind::current?.001:0)))));
