...
The GUI detaches views and releases large previous results in a background task. This avoids blocking Run while freeing millions of old samples. Result history grows in stable 1024-sample blocks, so appending no longer reallocates or moves the complete prefix and fixed-step preparation reserves only the small block table rather than one giant contiguous sample array. Random access, exact export and analysis semantics are unchanged. Results retain all recorded samples; no decimation is applied to recording or CSV.
