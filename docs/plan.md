...
Ядро иерархии реализовано и проверено: определения/экземпляры schema 7, public ports, числовые параметры, вложенность, flatten с картой происхождения, deep detach, grouping и история. Следующий шаг — визуальная интеграция иерархии; затем атомарные 2L/NPC. См. [контракт и проверки](hierarchy.md).
