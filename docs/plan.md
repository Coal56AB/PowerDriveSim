...
интерпретируемого C-кода инициализации и Gate и повторного прогона 33/33 CTest.
...
После приёмки M2, до моделей машин, первым шагом M3 выполнить общий signal layer
и пользовательский [«Блок кода»](code-block.md): редактируемые входы/выходы,
безопасное подмножество C, дискретное состояние, явный период и причинность через
последний принятый кадр. Это не переносит Controller/ADC/MCU/HAL из M6.
