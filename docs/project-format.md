# Формат проекта PowerDriveSim 22

UTF-8 (BOM допустим), LF или CRLF, текстовые записи, десятичные числа SI с точностью double. Первая строка — `PowerDriveSim 22`. Комментарии начинаются с `#` в первой колонке. Строки в кавычках используют escaping `std::quoted`: `\"` и `\\`.
...
Loader читает версии 1–22, writer записывает только 22:
