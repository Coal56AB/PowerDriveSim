# Формат проекта PowerDriveSim 21

UTF-8 (BOM допустим), LF или CRLF, текстовые записи, десятичные числа SI с точностью double. Первая строка — `PowerDriveSim 21`. Комментарии начинаются с `#` в первой колонке. Строки в кавычках используют escaping `std::quoted`: `\"` и `\\`.
...
Loader читает версии 1–21, writer записывает только 21:
