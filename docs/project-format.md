# Формат проекта PowerDriveSim 12

UTF-8 (BOM допустим), LF или CRLF, текстовые записи, десятичные числа SI с точностью double. Первая строка — `PowerDriveSim 12`. Комментарии начинаются с `#` в первой колонке. Строки в кавычках используют escaping `std::quoted`: `\"` и `\\`.
...
Loader читает версии 1–12, writer записывает только 12:
