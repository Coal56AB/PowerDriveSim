...
Выполнены атомарный редактор Qt 6, визуальная иерархия, раскрываемые 2L VSI / 3L NPC, топологическая диагностика и тёмная тема. Приёмка M1: docs/verification-m1-acceptance.md. В M2 добавлены источники sine/pulse/PWL с точными фронтами и schema 8, импорт CSV/TSV-таблиц с единицами SI, три проверенных RC-примера. Следующие задачи — piecewise-linear полупроводники, затем расширение библиотеки преобразователей. Таблицы встраиваются в проект, внешние файлы при запуске не нужны.
...
- Источники V/I: DC, sine, pulse, PWL; schema 8, loader v1..v8. Фронты вычисляются лениво, источник меняет RHS generic MNA. Проверены аналитика, одновременные source/gate events, UI и 16/16 CTest. См. docs/sources.md. Управление — записанные события и постоянный ШИМ, не Controller.
