...
int main(int argc, char **argv) {
    try {
        ...
        while (std::getline(lines, line))
            if (line.rfind("scope_enabled ", 0) != 0)
                v4 << line << '\n';
        std::istringstream old(v4.str());
        auto migrated = read_project(old);
        check(migrated.schema == 6 && !migrated.scope_enabled && migrated.plots.empty(),
        ...
        check(loaded.schema == 6 && loaded.plots.size() == 2 && !loaded.scope_enabled,
